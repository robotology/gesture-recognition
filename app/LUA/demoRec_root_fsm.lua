...
return rfsm.state {
   ...
   ST_CONNECTPORTS = rfsm.state{
           entry=function()
				   ...
				   yarp.NetworkBase_connect(demoRec_port:getName(), "/demoActionRecognition/rpc")
				   ret =  ret and yarp.NetworkBase_connect(gestRec_port:getName(), "/gest_rec/rpc")
                   if ret == false then
   ...
   ST_FINI = rfsm.state{
           entry=function()
                   ...
                   yarp.NetworkBase_disconnect(demoRec_port:getName(), "/demoActionRecognition/rpc")
				   yarp.NetworkBase_disconnect(gestRec_port:getName(), "/gest_rec/rpc")
                   ispeak_port:close()
